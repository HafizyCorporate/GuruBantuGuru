...
export default function Home() {
  ...
  // White Box ditingkatkan paddingnya untuk desktop
  ...
  return (
    <main className="relative w-full bg-black overflow-x-hidden">
      ...
      {/* SECTION 1: CANVAS - Scaling teks desktop */}
        ...
        <div className="fixed top-0 left-0 w-full h-screen z-0 overflow-hidden">
          <AnimatePresence>
            {!isLoaded && (
              <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }} className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-black">
                ...
                <p className="text-white text-[10px] lg:text-sm font-black tracking-[0.3em] uppercase opacity-50 italic">Fast Loading AI...</p>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center px-4 pointer-events-none">
            <motion.div style={{ opacity: text1Opacity }} className="absolute flex flex-col items-center w-full">
              ...
              <div className="mt-4 lg:mt-8 bg-black/90 px-4 py-1 lg:px-8 lg:py-2"><p className="font-bold tracking-[0.3em] uppercase text-[9px] md:text-xs lg:text-xl text-white">Asisten AI Untuk Para Guru Indonesia</p></div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
                ...
                <div className="bg-white rounded-[2.5rem] lg:rounded-[4rem] p-10 lg:p-16 shadow-2xl border border-blue-100 overflow-hidden flex flex-col">
                   ...
                   <a href="https://play.google.com/store/apps/details?id=com.soal.ai" target="_blank" rel="noopener noreferrer" className="w-fit hover:scale-105 transition-transform">
                      <img src="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" alt="Get it on Google Play" className="h-14 lg:h-20" />
                   </a>
                </div>
                ...
                <div className="bg-[#0f172a] rounded-[2.5rem] lg:rounded-[4rem] p-10 lg:p-16 shadow-2xl text-white overflow-hidden flex flex-col">
                   ...
                   <a href="https://play.google.com/store/apps/details?id=com.jawaban.ai" target="_blank" rel="noopener noreferrer" className="w-fit hover:scale-105 transition-transform bg-white/10 p-1 rounded-xl lg:rounded-2xl">
                      <img src="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" alt="Get it on Google Play" className="h-14 lg:h-20" />
                   </a>
                </div>
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-12 md:gap-24 lg:gap-40">
              <div className="flex flex-col items-center gap-6 lg:gap-10">
                ...
                <div className="flex gap-6 lg:gap-10">
                  <a href="#" className="w-12 h-12 lg:w-20 lg:h-20 bg-gray-50 rounded-2xl lg:rounded-[2rem] flex items-center justify-center hover:scale-110 transition-all border border-gray-100"><img src="/logo-fb.png" className="w-6 h-6 lg:w-10 lg:h-10 object-contain" /></a>
                  <a href="#" className="w-12 h-12 lg:w-20 lg:h-20 bg-gray-50 rounded-2xl lg:rounded-[2rem] flex items-center justify-center hover:scale-110 transition-all border border-gray-100"><img src="/logo-ig.png" className="w-6 h-6 lg:w-10 lg:h-10 object-contain" /></a>
                  <a href="#" className="w-12 h-12 lg:w-20 lg:h-20 bg-gray-50 rounded-2xl lg:rounded-[2rem] flex items-center justify-center hover:scale-110 transition-all border border-gray-100"><img src="/logo-threads.png" className="w-6 h-6 lg:w-10 lg:h-10 object-contain" /></a>
                </div>
              </div>
              <div className="flex flex-col items-center gap-6 lg:gap-10">
                <h4 className="text-[10px] lg:text-sm font-black tracking-[0.4em] text-blue-600 uppercase">Direct Message</h4>
                <div className="flex gap-6 lg:gap-10">
                  <a href="#" className="w-12 h-12 lg:w-20 lg:h-20 bg-gray-50 rounded-2xl lg:rounded-[2rem] flex items-center justify-center hover:scale-110 transition-all border border-gray-100"><img src="/logo-wa.png" className="w-6 h-6 lg:w-10 lg:h-10 object-contain" /></a>
                  <a href="#" className="w-12 h-12 lg:w-20 lg:h-20 bg-gray-50 rounded-2xl lg:rounded-[2rem] flex items-center justify-center hover:scale-110 transition-all border border-gray-100"><img src="/logo-email.png" className="w-6 h-6 lg:w-10 lg:h-10 object-contain" /></a>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-6 right-6 lg:bottom-12 lg:right-12 z-[100] flex flex-col items-end">
        <AnimatePresence>
          {isChatOpen && (
            <motion.div 
              ...
              className="mb-4 w-[320px] md:w-[380px] lg:w-[500px] bg-white rounded-2xl lg:rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden flex flex-col"
            >
              <div className="bg-blue-600 p-4 lg:p-7 text-white flex items-center gap-3 lg:gap-5">
                ...
                <div>
                  <h4 className="font-bold text-sm lg:text-xl">AI GuruBantuGuru</h4>
                  <p className="text-[10px] lg:text-xs opacity-80 uppercase tracking-widest">Online | Assistant</p>
                </div>
              </div>

              <div className="p-4 lg:p-7 h-[300px] lg:h-[450px] overflow-y-auto bg-gray-50 flex flex-col gap-3 lg:gap-5 scrollbar-hide">
                {chatMessages.map((msg, i) => (
                  <div 
                    key={i} 
                    className={`${msg.role === 'ai' ? 'bg-blue-100 text-blue-900 rounded-tl-none self-start' : 'bg-blue-600 text-white rounded-tr-none self-end'} text-xs lg:text-base p-3 lg:p-5 rounded-2xl lg:rounded-[1.8rem] max-w-[80%] shadow-sm leading-relaxed`}
                  >
                    {msg.text}
                  </div>
                ))}
                {isTyping && <div className="text-[10px] lg:text-xs text-gray-400 italic animate-pulse">AI sedang berpikir...</div>}
              </div>

              <div className="p-3 lg:p-6 bg-white border-t border-gray-100 flex gap-2 lg:gap-4">
                <input 
                  type="text" 
                  value={chatInput}
                  ...
                  className="flex-1 bg-gray-100 border-none rounded-full px-4 lg:px-6 py-2 lg:py-4 text-xs lg:text-base focus:ring-2 focus:ring-blue-500 outline-none text-black" 
                />
                <button 
                  onClick={handleSendMessage}
                  className="bg-blue-600 text-white p-2 lg:p-4 rounded-full hover:bg-blue-700 transition-colors"
                >
                  <svg viewBox="0 0 24 24" className="w-4 h-4 lg:w-7 lg:h-7 fill-current"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>
                </button>
              </div>
            </motion.div>
          )}
        ...
        <button 
          ...
          className="w-16 h-16 lg:w-24 lg:h-24 bg-blue-600 rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-transform active:scale-95 group"
        >
          {isChatOpen ? (
            <span className="text-white text-2xl lg:text-4xl font-light">✕</span>
          ) : (
            <div className="relative">
              <span className="absolute -top-1 -right-1 w-4 h-4 lg:w-6 lg:h-6 bg-red-500 rounded-full border-2 border-white animate-bounce"></span>
              <svg className="w-8 h-8 lg:w-12 lg:h-12 text-white fill-current" viewBox="0 0 24 24">
